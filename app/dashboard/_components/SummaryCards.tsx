...
export const AISummary: React.FC = () => {
  ...
  useEffect(() => {
    // simple rule-based summary (you can later replace with real AI API)
    const activeConditions = CONDITIONS.map((c) => c.name).join(", ");
    const medications = MEDICATIONS.map((m) => m.name).join(", ");

    const abnormalLabs = LAB_RESULTS.filter(
...
Patient ${USER_DATA.name} is a ${USER_DATA.age}-year-old ${USER_DATA.gender}.
Active conditions include: ${activeConditions || "none"}.
Currently taking: ${medications || "no medications"}.
Lab highlights: ${labText}.
Blood type: ${USER_DATA.bloodType}.
    `.trim();
